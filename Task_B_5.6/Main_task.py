...
# >>>>> БЛОК 0. (начало). ВВОДНЫЕ ПЕРЕМЕННЫЕ:
...
def current_gboard(list_): # Функция показывает вид текущего поля игры.
...
# >>>>> ПРОВЕРКА 1 (начало) - проверка совпадения всех символов по строке.
    ...
    for row in list_:  # Проходим по строкам
        for element_to_check in row:  # Проходим по символам в строке
            if element_to_check == simbol_x:  # если символ игрока 1 попадается,
                counter_simbol_x += 1  # то добавляем его в счетчик +1
...
# >>>>> ПРОВЕРКА 2 (начало) - проверка совпадения всех символов по диагонали.
def func_win_diog(list_):
    ...
    for row in list_:
        ...
        if counter_simbol_x == counter_row:
            print('---------------------------------------')
            print('Победил Игрок 1. Символы совпали по диагонали.')
            return True  # - нужно для условия для цикла While (см. БЛОК 4. КОНТРОЛЕР).
        else:
            ...
            if counter_simbol_o == counter_row:
                print('---------------------------------------')
                print('Победил Игрок 2. Символы совпали по диагонали.')
                return True  # - нужно для условия для цикла While (см. БЛОК 4. КОНТРОЛЕР).
        step += 1  # шаг задает смещение на 1 строку и 1 столбец каждую поледующую итерацию.
# <<<<< ПРОВЕРКА 2 (конец). ГОТОВО.

# --------------------------------------------------------------------------

# >>>>> ПРОВЕРКА 3 (начало) - проверка совпадения всех символов по обратной диагонали.
def func_win_diog_reverse(list_):
    list_reverse = []  # задаем переменную list_reverse,сюда будем сохранять развернутый справа-налево массив.
    ...
    result_3 = func_win_diog(list_reverse)  # - далее вызов ПРОВЕРКИ 2 (победа по диагонали) - слева-направо,
    # сверху-вниз.
    return result_3
...
# >>>>> ПРОВЕРКА 4 (начало) - совпадения всех символов по столбцу.
...
def func_win_column(list_):
    ...
    for index_column in range(counter_column):  # итерируем кол-во раз равное кол-ву столбцов (counter_column)
        for index_row in range(counter_row):  # итерируем кол-во раз равное кол-ву строк (counter_row)
            if list_[index_row][index_column] == simbol_x:  # проходимся по элементам 1го столбца,
                # если символ 'x', то счетчик +1
                counter_symbol_x += 1
            if counter_symbol_x == counter_row:  # если кол-во символов 'x' равно кол-ву строк - то победа.
                print('---------------------------------------')
                print('Победил Игрок 1. Символы совпали по столбцу.')
                return True  # - нужно для условия для цикла While (см. БЛОК 4. КОНТРОЛЕР).

            elif list_[index_row][index_column] == simbol_o:  # проходимся по элементам 1го столбца,
                # если символ 'o', то счетчик +1
                counter_simbol_o += 1
            if counter_simbol_o == counter_row:  # если кол-во символов 'o' равно кол-ву строк - то победа.
                ...
                return True  # - нужно для условия для цикла While (см. БЛОК 4. КОНТРОЛЕР).
        counter_symbol_x = 0  # Если в столбце не совпали все символы "x", обнуляем счетчик "x".
...
# Считаем количество строк и столбцов в массиве.
counter_motion = 0
counter_row = len(gb)
counter_column = 0
for i in gb:
    pluser = i.count("-")  # - расчет ходов по поиску поля "-" (по пустому полю).
    counter_motion += pluser  # - суммируем к счетчик ходов.
    if len(i) >= counter_column and len(gb) == counter_row:  # если длина списка в массиве и равно количеству
        # строк (чтобы матрица была квадратной), то ...
...
def check_add_row_id():  # - защита ввода индекса строки отличного от цифры и от попадания за диапазон строки.
    try:
        ...
        print('Ошибка ввода.')
        result_1 = check_add_row_id()  # - запускаем функцию заново, пока не введет правильное значение.
        return result_1
    else:
        return add_row

def check_add_column_id():  # - защита ввода индекса столбца отличного от цифры и от попадания за диапазон строки.
    try:
...
def add_simbol(simbol):
    ...
    if [add_row, add_column] in previous_motion:  # здесь проверка на заполнение в пустое поле.
        ...
        # Это далее пригодится в цикле while (см. БЛОК 4).

# <<<<< БЛОК 3. (конец). ГОТОВО!
# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
# \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\

# >>>>> БЛОК 4. (начало). КОНТРОЛЕР. Переключает игроков, в зависимости от общего количества ходов, по очереди.
...
while n >= 0:  # Основной цикл. Работает, пока не закончатся ходы.
    ...
    if counter > 0 and counter % 2 > 0:  # Если кол-во ходов нечетное,
        ...
        result = add_simbol(simbol)  # Передача True из БЛОКА 3, если символ вводился в свободное поле.
        if result:
            print('---------------------------------------')
            current_gboard(gb)  # Запускаем функцию из БЛОКА 1 - обновление текущего поля игры, его вывод.
            if func_win_line(gb) or func_win_diog(gb) or func_win_diog_reverse(gb) or func_win_column(gb) \
               or ((counter-1) == 0):  # Условия проверки выигрыша игроков.Запускаюся функции из БЛОКА ПРОВЕРОК.
            ...
            current_gboard(gb)
            print('---------------------------------------')
    elif counter > 0 and counter % 2 == 0:  # Если кол-во ходов четное,
        # то начинает ходить Игрок 2. Далее переключение по очереди.
